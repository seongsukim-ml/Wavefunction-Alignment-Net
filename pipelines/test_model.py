...
# cur_dir = os.path.dirname(os.path.abspath(__file__))
# sys.path.append(os.path.join(cur_dir, "..", "src"))
sys.path.insert(0, '/home/weixinran/erpailuo/se_3_equivariant_prediction/src')
...
from madftnn.utility.loadfile import LoadFromFile, LoadFromCheckpoint, save_argparse
...
def cli(config: DictConfig) -> None:
    ...
    if config.job_id == "auto":
        ...
        config.job_id = f'{ct.year}_{ct.month}_{ct.day}_{ct.hour}_{ct.minute}_{ct.second}_random{random.randint(0,100)}'
...
def main(config):
    ...
    trainer.test(model, data, ckpt_path="/home/weixinran/erpailuo/se_3_equivariant_prediction/amlt/equiv2-sym-qh9/step=49209-epoch=34-val_loss=0.4835191071033478.ckpt")
