...
def increment_path(path, exist_ok=False, sep='', args=None):

    if os.path.exists(os.path.join(args.log_dir, path)) and not exist_ok:

        for n in range(2, 9999):
            new_path = f'{path}{sep}{n}' 
            if not os.path.exists(os.path.join(args.log_dir, new_path)):  
                break
    else:
        new_path = path         
    return new_path