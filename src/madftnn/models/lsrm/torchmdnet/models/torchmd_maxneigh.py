...
class TorchMD_NeurIPS_Max(nn.Module):
    ...
    def __init__(
        ...
        ) # Neighbor embedding is just CFConv in SchNet

        self.attention_layers = nn.ModuleList()
        for _ in range(num_layers - 1):
            layer = EquivariantMultiHeadAttention_Max_Neigh(
                hidden_channels,
                distance_influence,
                num_heads,
                act_class,
        ...
        self.attention_layers.append(EquivariantMultiHeadAttention_Max_Neigh(
                hidden_channels,
                distance_influence,
                num_heads,
                act_class,
    ...
    def forward(self, 
        ...
        x = self.embedding(z) if x is None else x
        if self.otf_graph:
        ...
        if self.neighbor_embedding is not None and x is None:
            x = self.neighbor_embedding(z, x, edge_index, edge_weight, edge_attr)

        vec = torch.zeros(x.size(0), 3, x.size(1), device=x.device) if vec is None else vec
...
# ##TODO: ddp reduce.
# class All_Batch_Norm(nn.Module):
#     def __init__(self,normalized_shape,L2_norm_dim = None,momentum = 0.1,eps=1) -> None:
#         super().__init__()
        
#         self._a = nn.parameter.Parameter(torch.ones(normalized_shape)*1.,requires_grad=True)
#         self._b = nn.parameter.Parameter(torch.ones(normalized_shape)*0.,requires_grad=True)
        
        
#         self.mean = nn.parameter.Parameter(torch.ones(normalized_shape)*0.,requires_grad=False)
#         self.std = nn.parameter.Parameter(torch.ones(normalized_shape)*1.,requires_grad=False)
        
#         self.normalized_shape = normalized_shape 
#         self.L2_norm_dim = L2_norm_dim
#         self.momentum = momentum
#         self.eps = eps
        
#     def forward(self,fea):
#         # print(fea.shape)
#         if self.L2_norm_dim is not None:
#             x = torch.norm(fea,dim = self.L2_norm_dim)
#         else:
#             x = fea
#         if len(x.shape)>2:assert(False)
#         if self.training:
#             cur_mean = torch.mean(x,dim = 0)
#             cur_std = torch.std(x,dim = 0)
#             self.mean.data = (1-self.momentum)*self.mean.data+self.momentum*cur_mean
#             self.std.data = (1-self.momentum)*self.std.data+self.momentum*cur_std
#             # print(torch.mean(self.mean.data).item(),torch.mean(self.std.data).item(),torch.mean(cur_mean).item(),torch.mean(cur_std).item())
#         # print(self.mean,self.std)
#         fea = (fea-self.mean)/(self.std+self.eps)*self._a+self._b
#         return fea
            

class All_Batch_Norm(nn.Module):
    def __init__(self,normalized_shape,L2_norm_dim = None,momentum = 0.1,eps=1e-5) -> None:
        super().__init__()
        if(momentum!=0.1 or eps!=1e-5):assert(False)
        ...
        self.L2_norm_dim = L2_norm_dim
        if self.L2_norm_dim is not None:
            if self.L2_norm_dim <0:
                assert(False)
        self.soft_update_rho = momentum
        self.eps = eps
    ...
    def forward(self,fea):
        ...
        if self.L2_norm_dim is not None:
            ...
            if len(x.shape)>2:assert(False)
            norm_x = self.bn(x)
            norm_x = torch.unsqueeze(norm_x,dim = self.L2_norm_dim)
            x = torch.unsqueeze(x,dim = self.L2_norm_dim)
            fea = fea*norm_x/x
        else:
            x = fea
            if len(x.shape)>2:assert(False)
...
#         # if self.training:
#         #     cur_mean = torch.mean(x,dim = 0)
#         #     cur_var = torch.var(x,dim = 0)
#         #     self.mean.data = self.soft_update_rho*self.mean.data+(1-self.soft_update_rho)*cur_mean
#         #     self.std.data = self.soft_update_rho*self.std.data+(1-self.soft_update_rho)*cur_std
#         #     # print(torch.mean(self.mean.data).item(),torch.mean(self.std.data).item(),torch.mean(cur_mean).item(),torch.mean(cur_std).item())
#         # # print(self.mean,self.std)
#         # fea = (fea-self.mean)/(self.std+self.eps)*self._a+self._b
#         # return fea
...
class EquivariantMultiHeadAttention_Max_Neigh(MessagePassing):
    def __init__(
        self,
        hidden_channels,
        distance_influence,
        ...
        self.norm_fac_scalar  = nn.parameter.Parameter(torch.ones(1)*1.,requires_grad=False)
        self.norm_fac_vec  = nn.parameter.Parameter(torch.ones(1)*1.,requires_grad=False)
    ...
    def forward(self, x, vec, edge_index, r_ij, f_ij, d_ij):
        ...
        vec_dot = (vec1 * vec2).sum(dim=1) # vec1 \cdot vec2
    ...
    def message(self, q_i, k_j, v_j, vec_j, dk, dv, r_ij, d_ij):
        # attention mechanism
        if dk is None:
            attn = (q_i * k_j).sum(dim=-1) # num_edges * num_heads 
        else:
...
class EquivariantMultiHeadAttention_woEdgeUpdate(MessagePassing):
    def __init__(
        self,
        hidden_channels,
        distance_influence,
