...
class TensorProductRescale(torch.nn.Module):
    def __init__(self,
        ...
        if use_sparse_tp:
            self.tp = Sparse_TensorProduct(irreps_in1=self.irreps_in1,
...
class LinearRS(FullyConnectedTensorProductRescale):
    def __init__(self, irreps_in, irreps_out, bias=True, rescale=True, use_sparse_tp=False):
        super().__init__(irreps_in, o3.Irreps('1x0e'), irreps_out, 
            bias=bias, rescale=rescale, internal_weights=True, 
            shared_weights=True, normalization=None, use_sparse_tp=use_sparse_tp)
